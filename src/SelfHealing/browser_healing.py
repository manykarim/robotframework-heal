...
from .llm_client import LLM_TEXT_MODEL
...
class BrowserHealer:
    ...
    def __init__(self, instance=None, **kwargs):
        ...
        self.use_llm_for_locator_proposals = kwargs.get(
    ...
    def is_element_not_ready(self, message) -> bool:
        return (
            "element is not visible" in message
            or "waiting for element to be" in message
        )
    ...
    def wait_until_page_is_ready(self, timeout=20):
        BuiltIn().wait_until_keyword_succeeds(
            timeout, "1s", "Browser.Wait For Load State", "load", "1s"
        )
    ...
    def close_modal_dialog(self):
        ...
        prompt_content = {"page_source": str(dialog)}
        ...
        messages.append(
            {"role": "user", "content": (f"'page_source': ```{str(dialog)}```\n")}
        )
        ...
        while not locator_has_been_fixed:
            ...
            try:
                if self.browser.get_element_count(retry_selector) == 1:
                    ...
                    logger.info(
                        f"Locator to close dialog has been found: {retry_selector}",
                        also_console=True,
                    )
    ...
    def get_fixed_locator(self, data, result) -> str:
        ...
        try:
            shadowdom_exists = self.browser.evaluate_javascript(
                None, shadowdom_exist_script
            )
            if shadowdom_exists:
                soup = BeautifulSoup(
                    self.browser.evaluate_javascript(None, script), "html.parser"
                )
            else:
                soup = BeautifulSoup(self.browser.get_page_source(), "html.parser")
        except:
        ...
        if self.use_llm_for_locator_proposals:
            fixed_locator_list = self.get_locator_proposals_from_llm(
                data, result, source
            )
        else:
            fixed_locator_list = self.get_locator_proposals_from_parser(
                data, result, source
            )
        ...
        if self.use_locator_db:
                ...
                try:
                    ...
                    if self.browser.get_element_count(retry_selector) == 1:
                        try:
                            ...
                            original_locator_info = locator_db.search(
                                Query().locator
                                == failed_locator & Query().testsuite
                                == testsuite
                            )[-1]
                            added, removed, modified, same, similarity = compare_dict(
                                original_locator_info, retry_locator_info
                            )
                        except:
                            similarity = 0.01
                        fixed_locators_with_similarity.append(
                            (retry_selector, similarity)
                        )
                except:
                    continue
            ...
            while index < len(fixed_locator_list):
                try:
                    retry_selector = get_locator_with_prefix(
                        fixed_locator_list[index].strip()
                    )
                    ...
                    if self.browser.get_element_count(retry_selector) == 1:
                        ...
                        if "visible" in states:
                            retry_locator_info = self.get_locator_info(
                                retry_selector,
                                read_clickable_info=self.read_clickable_info,
                            )
                            retry_locator_info = {
                                key: value
                                for key, value in retry_locator_info.items()
                                if key not in ["testsuite"]
                            }
                            fixed_locators_with_info.append(
                                {
                                    "index": int(index),
                                    "fixed_locator": retry_selector,
                                    "additional_info": retry_locator_info,
                                }
                            )
                    elif self.browser.get_element_count(retry_selector) > 1:
                        if (
                            self.browser.get_element_count(f"{retry_selector}:visible")
                            == 1
                        ):
            ...
            if len(fixed_locators_with_info) > 50:
                fixed_locators_with_info = filter_locator_list_with_fuzz_median(
                    fixed_locators_with_info, failed_locator
                )
            ...
            logger.info(
                f"Pre-filtered locator candidates with info: {pprint.pformat(fixed_locators_with_info)}",
                also_console=False,
            )
            ...
            match data.name:
                ...
                case "Click" | "Click With Options":
                    ...
                    if self.read_clickable_info:
                        fixed_locators_with_info = [
                            x
                            for x in fixed_locators_with_info
                            if x["additional_info"]["clickable"] == True
                        ]
                    ...
                    messages.append(
                        {
                            "role": "user",
                            "content": (
                                "fixed_locators with `select` elements have a priority"
                                "Always return the unchanged fixed_locator."
                            ),
                        }
                    )
            ...
            logger.info(
                f"2nd LLM response with sorted candidates: {solution_text}",
                also_console=False,
            )
            ...
            if isinstance(sorted_retry_locators, dict):
                if "index" in sorted_retry_locators:
                    try:
                        retry_locator = next(
                            item["fixed_locator"]
                            for item in fixed_locators_with_info
                            if int(item["index"]) == int(sorted_retry_locators["index"])
                        )
                    except:
    ...
    def get_locator_info(self, locator, read_clickable_info=False):
        ...
        if read_clickable_info:
            ...
            if locator_info["tagName"] in clickable_tags:
                ...
                if (
                    self.does_cursor_pointer_style_exist(locator)
                    or self.does_checked_property_exist(locator)
                    or self.does_clickable_control_property_exist(locator)
                    or self.does_value_property_exist(locator)
                ):
                    locator_info["clickable"] = True
                else:
    ...
    def does_value_property_exist(self, locator):
        try:
            return (
                self.browser.evaluate_javascript(locator, f"(elem) => elem.{'value'}")
                == "on"
                or self.browser.evaluate_javascript(
                    locator, f"(elem) => elem.{'value'}"
                )
                == "off"
            )
        except:
    ...
    def does_checked_property_exist(self, locator):
        try:
            return (
                self.browser.evaluate_javascript(locator, f"(elem) => elem.{'checked'}")
                != ""
            )
        except:
            return False
    ...
    def does_clickable_control_property_exist(self, locator):
        try:
            tag = self.browser.evaluate_javascript(
                locator, f"(elem) => elem.{'control.tagName'}"
            )
            if tag == "BUTTON" or tag == "A":
                return True
            elif tag == "INPUT":
                type = self.browser.evaluate_javascript(
    ...
    def get_property_value(self, locator, property):
        try:
            return self.browser.evaluate_javascript(
                locator, f"(elem) => elem.{property}"
            )
        except:
            return None
    ...
    def get_locator_proposals_from_llm(self, data, result, source):
        ...
        while not locator_has_been_fixed:
            ...
            logger.info(
                f"1st LLM response: {pprint.pformat(solution_text)}\n",
                also_console=False,
            )
    ...
    def get_locator_proposals_from_parser(self, data, result, source):
        ...
        if PARALLEL:
            ...
            with ProcessPoolExecutor() as executor:
                ...
                futures = [
                    executor.submit(get_locators_for_element_type, item, str(soup))
...
def get_locator_with_prefix(locator):
    ...
    if locator:
        ...
        locator_with_prefix = locator_with_prefix.replace(
            ":-soup-contains-own", ":text"
        )
        locator_with_prefix = locator_with_prefix.replace(
            ":-soup-contains", ":has-text"
        )
