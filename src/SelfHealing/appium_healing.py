...
class AppiumHealer:
    ...
    def is_permission_popup_open(self) -> bool:
        try:
            if (
                len(
                    self.appium.find_elements(
                        AppiumBy.XPATH, "//*[contains(@resource-id,'dialog_container')]"
                    )
                )
                == 1
            ):
    ...
    def confirm_permission_popup(self) -> bool:
        ...
        for button in buttons:
            try:
                elem = self.appium.find_element(
                    AppiumBy.XPATH, f"//*[contains(@text, '{button}')] "
                )
                elem.click()
                break
            except:
                pass
        logger.error("Popup Dialog could not be closed")
        ...
        for soup in soup_list:
            if self.use_llm_for_locator_proposals:
                fixed_locator_list += self.get_locator_proposals_from_llm(
                    data, result, str(soup.hierarchy)
                )
            else:
            ...
            fixed_locator_list = [
                item for item in fixed_locator_list if item not in existing_locators
            ]
            for retry_selector in fixed_locator_list:
                try:
                    retry_locator_info = self.get_locator_info(
                        str(soup.hierarchy), retry_selector
                    )
        ...
        def add_elements(soup, elements):
            for element in elements:
                if not any(
                    ...
                    for existing_element in soup.find_all(element.name)
                ):
                    soup.append(element)
    ...
    def get_locator_proposals_from_llm(self, data, result, source):
        ...
        schema = {"fixed_locator": "The fixed xpath locator."}
